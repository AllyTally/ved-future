...
future_teleporter_tool_id = AC_register_tool("Teleporter", "tele", "s2", 14, { 0, 0, 11, 11 },
    function(atx, aty, roomx, roomy)
...
AC_register_click_handler(function(atx, aty, mouse_x, mouse_y)
    if (not FUTURE_DRAGGING) and selectedtool == future_teleporter_tool_id then
...
AC_register_entity_render_handler(function(entity, type, x, y, k, interact, offsetx, offsety, myroomx, myroomy,
                                           scriptname_args, nthscriptbox)
    if type == 5 then
        v6_setcol(entity.p2)
    ...
    if type == 14 then
        ...
        if (selectedtool == future_teleporter_tool_id) then
            ...
            local curve = love.math.newBezierCurve(off_point_1[1], off_point_1[2], off_point_2[1], off_point_2[2],
                off_point_3[1], off_point_3[2])
            love.graphics.setColor(0, 255, 0)
            love.graphics.setLineWidth(2)
            love.graphics.line(curve:render())
            love.graphics.setColor(255, 255, 255)
            local mouse_x, mouse_y = (getlockablemouseX() - screenoffset), getlockablemouseY()
            local touching_point_1 = touching_points(mouse_x, mouse_y, off_point_2[1] - offsetx, off_point_2[2] - offsety) and
            2 or 1
            local touching_point_2 = touching_points(mouse_x, mouse_y, off_point_3[1] - offsetx, off_point_3[2] - offsety) and
            2 or 1
    ...
    if type == 20 or type == 21 or type == 22 or type == 23 or type == 24 then
        ...
        if type == 20 then
            if hovering_over_name(true, k, entity, offsetx, offsety, myroomx, myroomy) and interact then
                ...
                scriptname_args[1], scriptname_args[2], scriptname_args[3], scriptname_args[4] = true, k, entity,
                    nthscriptbox
            end
        end
        return true
